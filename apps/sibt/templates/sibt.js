...
var _willet_show_topbar = function() {
    ...
    bar_html = " " +
        ...
        "    <button id='yesBtn' class='yes' onclick='_willet_do_vote(1);'>" +
        "        Buy it "+
        "    </button> "+
        "    <button id='noBtn' class='no' onClick='_willet_do_vote(0);'> "+
        "        Skip it "+
        "    </button> "+
        "</div> "+
        "<div onclick='_willet_toggle_results()' id='_willet_toggle_results' class='toggle_results' style='display: none'> "+
        "    <span class='down'>" +
    ...
    if (_willet_show_votes && !_willet_has_voted && !_willet_is_asker) {
        ...
        _willet_topbar.find('div.message').html('Thanks for voting!').fadeIn();
        _willet_topbar.find('div.toggle_results').fadeIn();
    } else if (_willet_is_asker) {
